...
(function(undef){
	...
	var defer = function (alwaysAsync){
		var alwaysAsyncFn = (undef !== alwaysAsync ? alwaysAsync : defer.alwaysAsync) ? nextTick : function(fn){fn();}
			...
			, _promise  = {
						...
						function(value){
							try{
								if( isNotVal(fulfilled)){
									...
									d.resolve(isFunc(fulfilled) ? fulfilled(value) : (onlyFuncs ? value : fulfilled));
								}
							}catch(e){
								d.reject(e);
							}
				...
				, apply: function(fulfilled, failed){
					return this.then(
						function(a){ return isFunc(fulfilled) ? fulfilled.apply(null, isArray(a) ? a : [a]) : (onlyFuncs ? a : fulfilled);}
						, failed || undef
					);
				}
				 ...
				 *                      when the promise is not pendint anymore
				...
				/**
				 * @param {function} cb a callback called regardless of the fulfillment or rejection of the promise which will be called
				 *                      when the promise is not pendint anymore with error as first parameter if any as in node style
				...
				, nodify: function(cb){
					return this.then(
						function(a){
							return isFunc(cb) ? cb.apply(null, isArray(a) ? a.splice(0,0,undefined) && a : [undefined,a]) : (onlyFuncs ? a : cb);
						}
						, function(e){
							return cb(e);
						}
		...
		function _resolve(val){
			...
			try {
				...
				if ( val && isFunc(then) ) { // managing a promise
					if( val === _promise ){
	...
	*/
	...
	/**
	 * return a promise for the return value of function call which will be resolved in delay ms or rejected if given function throw an error
	 * @param {function} fn
	 ...
	 * @param {array|...*} [promise] can be a single array of promise/values as first parameter or a list of direct parameters promise/value (non promise values are promisified)
